                    ...
                    <div key={video.id} className="hover:bg-[#1e1e1e] rounded-lg cursor-pointer transition-all hover:scale-105">
                        ...
                        <div className="py-1 ps-2 flex justify-between items-start gap-2">
                            ...
                            <div className="relative">
                                ...
                                <button onClick={(e) => setSelectedVideoId((selectedVideoId === video.id) ? null : video.id)}
                                    className="w-6 hover:bg-[#3c3c3c] rounded-full shrink-0 cursor-pointer"
                                >
                                    <img src={more_options_icon} alt="" />
                                </button>

                                {/* Dropdown menu: Watch Later & Remove options */}
                                {(selectedVideoId === video.id) && (
                                    <div className="w-max py-2 bg-[#282828] rounded-md border border-white/10 text-sm whitespace-nowrap absolute top-full right-0 z-10">
                                        <div className="px-3 py-2 hover:bg-[#3e3e3e] cursor-pointer flex items-center gap-2">
                                            <img src={watch_later_icon} className="w-4" alt="" />
                                            <span>Add to Watch Later</span>
                                        </div>

                                        <div onClick={() => removeVideo(video.id)} className="px-3 py-2 hover:bg-[#3e3e3e] cursor-pointer flex items-center gap-2">
